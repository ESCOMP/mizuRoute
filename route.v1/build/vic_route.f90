...
!    write(*,'(a,1x,es14.7)') 'Sum of routed flow = ',RCHFLX(iEns,iSeg)%UPSBASIN_QR
  ...
  ! Structures modified:
  !  
  !   Updates structure RCHFLX%BASIN_QR_VIC (module reach_flux)
  !                     RCHFLX%REACH_Q_VIC
  ! ----------------------------------------------------------------------------------------
  USE reachparam
  USE reach_flux
  ...
  seg_loop: do iSeg=1,nSeg
    ...
    upstrms_loop: do iUps=1,nUps
      ...
      UH_data: do iTDH=1,nTDH
        RCHFLX(iens,iSeg)%QFUTURE_VIC(iTDH) = RCHFLX(iens,iSeg)%QFUTURE_VIC(iTDH) &
                                            + NETOPO(iSeg)%UH(iUps)%UH_DATA(iTDH)*RCHFLX(iEns,jUps)%UPSBASIN_QR
      enddo UH_data 
    enddo upstrms_loop 

    ! save the routed runoff
    RCHFLX(iEns,iSeg)%BASIN_QR_VIC(0) = RCHFLX(iEns,iSeg)%BASIN_QR_VIC(1)  ! (save the runoff from the previous time step)
    RCHFLX(iEns,iSeg)%BASIN_QR_VIC(1) = RCHFLX(iEns,iSeg)%QFUTURE_VIC(1)
    ! Add local routed flow
    RCHFLX(iEns,iSeg)%REACH_Q_VIC    = RCHFLX(iEns,iSeg)%BASIN_QR_VIC(1)+RCHFLX(iEns,iSeg)%BASIN_QR(1)
    ! move array back   use eoshift
    RCHFLX(iEns,iSeg)%QFUTURE_VIC=eoshift(RCHFLX(iEns,iSeg)%QFUTURE_VIC,shift=1)

  enddo seg_loop

  end subroutine conv_upsbas_qr

end module vic_route
