...
!  END INTERFACE
  ...
  INTERFACE beschb
    ...
    USE nrtype
    REAL(sP), INTENT(IN) :: x
    REAL(sP), INTENT(OUT) :: gam1,gam2,gampl,gammi
    END SUBROUTINE beschb_s
!BL
    SUBROUTINE beschb_v(x,gam1,gam2,gampl,gammi)
    USE nrtype
    REAL(sP), DIMENSION(:), INTENT(IN) :: x
    REAL(sP), DIMENSION(:), INTENT(OUT) :: gam1,gam2,gampl,gammi
...
!BL
  ...
  INTERFACE
    FUNCTION interp(uc)
    USE nrtype
    REAL(sP), DIMENSION(:,:), INTENT(IN) :: uc
    REAL(sP), DIMENSION(2*size(uc,1)-1,2*size(uc,1)-1) :: interp
    END FUNCTION interp
  END INTERFACE
  INTERFACE
    FUNCTION rank(indx)
  ...
  INTERFACE
    ...
    REAL(SP), DIMENSION(size(u,1),size(u,1)) :: lop
    END FUNCTION lop
  END INTERFACE
  INTERFACE
...
!BL
  ...
  INTERFACE
    ...
    USE nrtype
    REAL(SP), DIMENSION(:), INTENT(INOUT) :: cof
...
!BL
  ...
  INTERFACE
    ...
    REAL(SP), DIMENSION(:), INTENT(OUT) :: cof
    REAL(SP), INTENT(OUT) :: dev
    INTERFACE
      FUNCTION func(x)
      USE nrtype
      REAL(SP), DIMENSION(:), INTENT(IN) :: x
  ...
  INTERFACE ratval
    ...
    REAL(SP), INTENT(IN) :: x
    INTEGER(I4B), INTENT(IN) :: mm,kk
    REAL(SP), DIMENSION(mm+kk+1), INTENT(IN) :: cof
    REAL(SP) :: ratval_s
...
!BL
    ...
    INTEGER(I4B), INTENT(IN) :: mm,kk
    REAL(SP), DIMENSION(mm+kk+1), INTENT(IN) :: cof
    REAL(SP), DIMENSION(size(x)) :: ratval_v
...
!BL
  ...
  FUNCTION resid(u,rhs)
    USE nrtype
    REAL(SP), DIMENSION(:,:), INTENT(IN) :: u,rhs
    REAL(SP), DIMENSION(size(u,1),size(u,1)) :: resid
    END FUNCTION resid
  END INTERFACE
  INTERFACE rf
    FUNCTION rf_s(x,y,z)
...
!BL
  ...
  INTERFACE
    FUNCTION rstrct(uf)
    USE nrtype
    REAL(SP), DIMENSION(:,:), INTENT(IN) :: uf
    REAL(SP), DIMENSION((size(uf,1)+1)/2,(size(uf,1)+1)/2) :: rstrct
    END FUNCTION rstrct
  END INTERFACE
  INTERFACE
  ...
  INTERFACE
    ...
    REAL(SP), DIMENSION(3,3), INTENT(IN) :: rhs
  ...
  INTERFACE
    ...
    USE nrtype
    REAL(SP), DIMENSION(:,:), INTENT(IN) :: a,b,c,d,e,f
    REAL(SP), DIMENSION(:,:), INTENT(INOUT) :: u
    REAL(SP), INTENT(IN) :: rjac
    END SUBROUTINE sor
  END INTERFACE
  INTERFACE
    SUBROUTINE sort(arr)
...
! and put it on the next one after that.
  ...
  INTERFACE
    FUNCTION vander(x,q)
    USE nrtype
    REAL(SP), DIMENSION(:), INTENT(IN) :: x,q
    REAL(SP), DIMENSION(size(x)) :: vander
    END FUNCTION vander
  END INTERFACE
  INTERFACE
