...
!
  ...
  CONTAINS
    ...
    FUNCTION gageObs_constructor(fileName, &
      ...
      ! Currently netcdf read function take a file name and open netcdf instead of taking ncid
      ! TO-DO: if netcdf I/O functions are changed so that they read ncid, obs netcdf should be open here
      !        till the end of simulation (close in model_finalize subroutine (model_utils module)
    ...
    SUBROUTINE read_site(this, ierr, message, index_loc)
      ...
      call get_nc_dim_len(this%fileName, this%dimName_loc, nSite, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
      ...
      if (present(index_loc)) then
        ...
        call get_nc(this%fileName, this%varName_loc, array_temp, iStart, nCount, ierr, cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
        ...
        call get_nc(this%fileName, this%varName_loc, this%site, iStart, nCount, ierr, cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
      end if
    ...
    SUBROUTINE read_reach(this, ierr, message, index_loc)
      ...
      if (present(index_loc)) then
        ...
        call get_nc(this%fileName, this%varName_loc, array_temp, 1, nSite, ierr, cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
        ...
        call get_nc(this%fileName, this%varName_loc, this%reach, 1, nSite, ierr, cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
      end if
    ...
    SUBROUTINE read_time(this, ierr, message, index_time)
      ...
      call get_nc_dim_len(this%fileName, this%dimName_time, nTime, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif

      call get_var_attr(this%fileName, this%varName_time, 'units', timestr_start, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif

      call get_var_attr(this%fileName, this%varName_time, 'calendar', gage_calendar, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif

      call datetime_start%str2datetime(timestr_start, gage_calendar, ierr, cmessage)
      ...
      if (present(index_time)) then
        ...
        call get_nc(this%fileName, this%varName_time, timeVar_temp, 1, nTime, ierr, cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif

        do ix=1,nTime_read
          timeVar = timeVar_temp(index_time(ix))*convTime2Secs
          this%time(ix) = datetime_start%add_sec(timeVar, ierr, cmessage)
        end do
      else
        allocate(timeVar_temp(nTime), stat=ierr, errmsg=cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage)//' [this%time]'; return; endif
    ...
    SUBROUTINE read_obs(this, ierr, message, index_loc, index_time)
      ...
      if (present(index_loc)) then
        ...
        call get_nc_dim_len(this%fileName, this%dimName_loc, nLoc, ierr, cmessage)
        if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
        nLoc_read = nLoc
        ix_loc = 1
      end if
      ...
      call get_nc(this%fileName, this%varName_obs, array_temp, iStart, nCount, ierr, cmessage)
      if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
    ...
    SUBROUTINE sub_comp_link_reach2reach(this, reachID_in)
      ...
      this%link_index = match_index(reachID_in, this%reach, ierr, cmessage)
    ...
    SUBROUTINE sub_comp_link_site2reach(this, reachID_in, gageMeta_in)
      ...
      ! Find index of matching reachID in reachID_in array
      this%link_index = match_index(reachID_in, reachID_gage, ierr, cmessage)
