...
contains
 ...
 ! ***************************************************************
 ! public subroutine: Domain decomposition
 ! ***************************************************************
...
CALL system_clock(count_rate=cr)
   ...
   call decomposeDomain(structNTOPO, majorMainstem, maxSegs, ierr, cmessage)
...
call system_clock(startTime)
   ...
   do ix=1,nDomain
     ...
     nContribHRU=nContribHRU+sumHruLocal
     allocate(domains(ix)%hruIndex(sumHruLocal), stat=ierr)
...
write(*,"(A,1PG15.7,A)") '   elapsed-time [hru_decomposition] = ', elapsedTime, ' s'
 ...
 ! private subroutine:
 ! ***************************************************************
 subroutine decomposeDomain(structNTOPO, isMainstem, maxSegs, ierr, message)
   ! Details:
   ...
   do iOut = 1,nOut
     ...
     nDomain = nDomain + 1
     domains(nDomain)%basinType = tributary
     domains(nDomain)%segIndex  = structNTOPO(ixOutlets(iOut))%var(ixNTOPO%allUpSegIndices)%dat
   enddo
   ...
   domains(nDomain)%basinType = mainstem
   ...
   do iTrib = 1,nTrib
     ...
     domains(nDomain)%basinType = tributary
     domains(nDomain)%segIndex = structNTOPO(ixTribOutlet(iTrib))%var(ixNTOPO%allUpSegIndices)%dat
