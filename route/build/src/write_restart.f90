...
CONTAINS
 ...
 integer(i4b)                         :: ixDim_common(4)  ! NetCDF file ID
 ...
 do jDim = 1,size(ixDim_common)
  if (meta_stateDims(jDim)%dimLength == integerMissing) then
   call set_dim_len(jDim, ierr, cmessage)
   if(ierr/=0)then; message=trim(message)//trim(cmessage)//' for '//trim(meta_stateDims(jDim)%dimName); return; endif
  endif
  ierr = nf90_def_dim(ncid, trim(meta_stateDims(jDim)%dimName), meta_stateDims(jDim)%dimLength ,meta_stateDims(jDim)%dimId)
 end do
 ...
 CONTAINS
  ...
  SUBROUTINE set_dim_len(ixDim, ierr, message)
   ...
   select case(ixDim)
    ...
    case default; ierr=20; message=trim(message)//'unable to identify dimension variable index'; return
   end select

  END SUBROUTINE

  SUBROUTINE define_IRFbas_state(ierr, message)
   implicit none
   ! output
   integer(i4b), intent(out)  :: ierr          ! error code
   character(*), intent(out)  :: message       ! error message
   ...
   ! initialize error control
   ierr=0; message='define_IRFbas_state/'
   ...
   if (meta_stateDims(ixStateDims%tdh)%dimLength == integerMissing) then
     ...
     if(ierr/=0)then; message=trim(message)//trim(cmessage)//' for '//trim(meta_stateDims(ixStateDims%tdh)%dimName); return; endif
   end if
   ...
   do iVar=1,nVarsIRFbas
    select case(iVar)
     ...
     case default; ierr=20; message=trim(message)//'unable to identify hill-slope routing state variable index'; return
    end select
    if(ierr/=0)then; message=trim(message)//trim(nf90_strerror(ierr)); return; endif
   end do
  end associate
  ...
  SUBROUTINE define_KWT_state(ierr, message)
   implicit none
   ! output
   integer(i4b), intent(out)  :: ierr        ! error code
   character(*), intent(out)  :: message     ! error message
   ...
   ! initialize error control
   ierr=0; message='define_KWT_state/'
   ...
   if (meta_stateDims(ixStateDims%wave)%dimLength == integerMissing) then
     ...
     if(ierr/=0)then; message=trim(message)//trim(cmessage)//' for '//trim(meta_stateDims(ixStateDims%wave)%dimName); return; endif
   end if
   ...
   do iVar=1,nVarsKWT
    ...
    select case(iVar)
     ...
     case default; ierr=20; message=trim(message)//'unable to identify KWT routing state variable index'; return
    end select
    if(ierr/=0)then; message=trim(message)//trim(nf90_strerror(ierr)); return; endif
  ...
  SUBROUTINE define_IRF_state(ierr, message)
   implicit none
   ! output
   integer(i4b), intent(out)  :: ierr        ! error code
   character(*), intent(out)  :: message     ! error message
   ...
   ! initialize error control
   ierr=0; message='define_IRF_state/'
   ...
   if (meta_stateDims(ixStateDims%tdh_irf)%dimLength == integerMissing) then
     ...
     if(ierr/=0)then; message=trim(message)//trim(cmessage)//' for '//trim(meta_stateDims(ixStateDims%tdh_irf)%dimName); return; endif
   endif
   ...
   do iVar=1,nVarsIRF
    ...
    select case(iVar)
     ...
     case default; ierr=20; message=trim(message)//'unable to identify IRF routing state variable index'; return
    end select
    if(ierr/=0)then; message=trim(message)//trim(nf90_strerror(ierr)); return; endif
 ...
 CONTAINS
  ...
  SUBROUTINE write_IRFbas_state(ierr, message)
  implicit none
  ! output
  integer(i4b), intent(out)  :: ierr            ! error code
  character(*), intent(out)  :: message         ! error message
  ...
  ! initialize error control
  ierr=0; message='write_IRFbas_state/'
  ...
  do iVar=1,nVarsIRFbas
   ...
   select case(iVar)
    ...
    case default; ierr=20; message=trim(message)//'unable to identify basin routing variable index'; return
   end select
   if(ierr/=0)then; message=trim(message)//'problem allocating space for basin IRF routing state '//trim(meta_irf_bas(iVar)%varName); return; endif
 ...
 ! --Convert data structures to arrays
   ...
   do iSeg=1,nSeg
     do iVar=1,nVarsIRFbas
      ...
      select case(iVar)
       ...
       case default; ierr=20; message=trim(message)//'unable to identify basin IRF state variable index'; return
      end select
  ...
  do iVar=1,nVarsIRFbas
   ...
   select case(iVar)
    ...
    case default; ierr=20; message=trim(message)//'unable to identify basin IRF variable index for nc writing'; return
   end select
   if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
  ...
  SUBROUTINE write_KWT_state(ierr, message)
  implicit none
  ! output
  integer(i4b), intent(out)  :: ierr            ! error code
  character(*), intent(out)  :: message         ! error message
  ...
  ! initialize error control
  ierr=0; message='write_KWT_state/'
  ...
  do iVar=1,nVarsKWT
    ...
    if(ierr/=0)then; message=trim(message)//'problem allocating space for KWT routing state '//trim(meta_kwt(iVar)%varName); return; endif
  end do
  ...
  do iVar=1,nVarsKWT
    ...
    select case(iVar)
     ...
     case default; ierr=20; message=trim(message)//'unable to identify IRF variable index for nc writing'; return
    end select
   if(ierr/=0)then; message=trim(message)//trim(cmessage); return; endif
  ...
  SUBROUTINE write_IRF_state(ierr, message)
  implicit none
  ! output
  integer(i4b), intent(out)  :: ierr            ! error code
  character(*), intent(out)  :: message         ! error message
  ...
  ! initialize error control
  ierr=0; message='write_IRF_state/'
  ...
  if(ierr/=0)then; message=trim(message)//'problem allocating space for numQF'; return; endif
  ...
  do iVar=1,nVarsIRF
   ...
   if(ierr/=0)then; message=trim(message)//'problem allocating space for IRF routing state '//trim(meta_irf(iVar)%varName); return; endif
  end do
