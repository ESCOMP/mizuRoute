...
type(all_points), allocatable :: hru2seg(:)          ! hru-to-segment mapping structure
type(RCHTOPO),    allocatable :: NETOPO(:)           ! River Network topology
type(RCHPRP),     allocatable :: RPARAM(:)           ! Reach parameters

! define desired reaches
integer(i4b)                  :: nHRU                ! number of HRUs
integer(i4b)                  :: nRch                ! number of desired reaches
logical(lgt) , allocatable    :: isReachDesired(:)   ! logical vector to define if reaches are desired
...
print*, 'PAUSE: after read control'; read(*,*)
          ...
          !          ! check if the outlet reach is defined
          !          if(idSegOut>0)then
          !
          !           ! define the logical vector to get the desired reaches
          !           allocate(isReachDesired(nRch), stat=ierr)
          !           if(ierr/=0) call handle_err(ierr, 'unable to allocate space for the logical vector of desired reaches')
          !
          !           ! identify all reaches upstream of a given reach
          !           call reach_mask(&
          !          				 ! input
          !          				 idSegOut,  &      ! input: reach index
          !          				 NETOPO,    &      ! input: network topology structures
          !          				 nRch,      &      ! input: number of reaches
          !          				 ! output
          !          				 isReachDesired,&  ! output: flags of desired reaches
          !          				 ierr, cmessage )  ! output: error control
          !           if(ierr/=0) call handle_err(ierr, cmessage)
          !
          !           print*, 'isReachDesired = ', isReachDesired
          !           print*, 'PAUSE: '; read(*,*)
          !
          !          endif
...
stop
    ...
    !          	if (NETOPO(iSeg)%REACHID == idSegOut) then
