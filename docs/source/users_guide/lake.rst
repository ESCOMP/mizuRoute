...
mizuRoute also has the capability, in addition to the parameterix model simulation for a lake or reservoir, to accound for a time series of abstraction or injection of water into a lake. This capabilty allows more flexible mdoeling for the reservoir with two outlets for example (one can be simulated with parameteric formulation, while the second can be emulated using a target realse timesers). additinally the more complex hydrological fluxes such as interaction of lake and reserovir water balance with external hydrological and groundwatetre level can be captured in this model. For using of this approach the water managemtn optional intpu files and their control keys should be activate :ref:`WaterManagement_file`.
